...
func AddOnes(Data PPM) []map[string]interface{} {
	...
	if Data.PPM.No_ppm != "" {
		...
		o := orm.NewOrm()
		sql := "UPDATE tblfcossppm SET ppm_no = ?, ppm_sign_nip = ?, ppm_date = ?, ppm_sign_name = ?, ppm_sign_date = ?, ppm_sign_position = ?, ppm_sign_ga = ? WHERE oss_id = ?"
		res, err := o.Raw(sql, Data.PPM.No_ppm, Data.PPM.Nip_ttd_ppm, Data.PPM.Tgl_ppm, Data.PPM.Nama_ttd_ppm, Data.PPM.Tgl_ttd_ppm, Data.PPM.Jabatan_ttd_ppm, `06`, Data.PPM.Oss_id).Exec()
	...
	*/
	...
	if bol == false {
		...
		//select OSS ID
		var maps []orm.Params
		sql := "SELECT * FROM tblfcossppm WHERE oss_id = ?"
		num, _ := o.Raw(sql, Data.PPM.Oss_id).Values(&maps)

		if num == 0 {

			bole, er, _ := InsertPPM(Data)
			if bole == false {
				row["ResponseCode"] = "99"
				row["ResponseDescription"] = er.Error()
				list[0] = row
